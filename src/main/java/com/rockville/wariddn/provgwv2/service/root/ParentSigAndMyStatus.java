...
public class    ParentSigAndMyStatus extends Operation {
            ...
            if (vnd != null && vnd.getVirtualMSISDN() > 0 && vnd.getStatus() > 0 && vnd.getStatus() < 4) {
                ...
                if (!msg.isEmpty()) {
                    ...
                    try {
                        ...
                        if (request.isSigRequest()) // if request is to set signature
                        {
                            user.setSmsSignature(msg);
                        } else if (request.isStatusRequest()) // if request is to set mystatus
                        {
                            user.setMyStatus(msg);
                        }
                        ...
                        sms_string_after_replacing_params = constants.getMESSAGE_TEMPLATE().get("MSG_SIG_SET_" + request.getLanguage());
